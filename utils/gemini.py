...
def generate_followup_prompt(previous_output):
    """
    Prompt Chaining to Avoid Truncated Response by Gemini Flash 2.0

    Arguments:
    previous_output (str): JSON response from previous run

    Output:
    JSON response
    """
    followup_prompt = f"""
    Continue from where you left off in the previous JSON response.
    Do not re-write the entire JSON response. Just continue from where you left off.
...
def get_full_response_recursively(client, image, initial_prompt, token_limit=8000, accumulated_text=""):
    """
    Recursively get the full response from Gemini if it exceeds the token limit.

    Arguments:
    - client (object): Your API client
    - image (any): The image you're sending as part of contents (PIL, binary, etc.)
    - initial_prompt (str): The main OCR prompt
    - token_limit (int): Threshold to check for truncation (default=8000)
    - accumulated_text (str): Accumulated text from previous truncated calls (used in recursion)

    Returns:
    - str: The (eventually) full text that doesn't exceed the token limit
    """
    # Call Gemini with the current prompt
    response = client.models.generate_content(
    ...
    # Clean the triple-backtick fences, if present
    current_output = response.text.replace("```json\n", "").replace("```", "").strip()

    # Accumulate the text so far
    accumulated_text += current_output

    # Check if truncated
    num_output_token = response.usage_metadata.candidates_token_count
    if num_output_token > token_limit:
        print("Token limit exceeded again; generating follow-up prompt...")

        # Create a follow-up prompt from the accumulated text so far
        # (Define generate_followup_prompt to suit your needs.)
        followup_prompt = generate_followup_prompt(accumulated_text)

        # Recursively call to get the next chunk
        return get_full_response_recursively(
            client=client,
            image=image,
        ...
        # If we’re no longer truncated, return the accumulated text
        return accumulated_text


def run_gemini_on_image(image_path, client, prompt):
    """
    Run Gemini Flash 2.0 for OCR and convert output to JSON.

    Arguments:
    - image_path (list): Path to image
    - client (object): Your Gemini API client
    - folder_output (str): Path to folder to store JSON/TXT files
    - prompt (str): The initial OCR prompt

    Output:
    - Saves a JSON file in folder_output if possible; otherwise saves as TXT.
    """
    # 1. Read image
    image = Image.open(image_path)

    # 2. Get the full text recursively (handles multiple truncations).
    full_output = get_full_response_recursively(
        client=client,
        image=image,
        initial_prompt=prompt,
        token_limit=30000,
        accumulated_text=""  # start empty
    )

    # 3. Define output paths
    os.makedirs('JSON', exist_ok=True)
    json_output_path = 'JSON/' + image_path.split('/')[-1].split('.jpg')[0] + '.json'
    txt_output_path = 'JSON/' + image_path.split('/')[-1].split('.jpg')[0] + '.txt'
    ...
    try:
        ...
        with open(txt_output_path, 'w', encoding='utf-8') as txt_file:
            txt_file.write(full_output)
        print(f"[FAIL] Invalid JSON. Saved as {txt_output_path}\n")
